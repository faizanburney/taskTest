...
This project uses Basic Auth for authentication:

username: "user"

password: "password"

these fields can be changed from application.properties file.


1) Crud for car implemented.
2) Car and driver association implemented now in Driver details you can see car details if car is assigned to the 
driver.
3) Filtering implemented on driver controller that generates filter query dynamically on runtime.

    `http://localhost:8080/v1/drivers/advanced?search=username:driver,car.rating>3`

    This will get a list of all drivers with name like "driver" with above 3 rating.
    ...
    * : – contains(string properties)/equals(non string properties).

    Drivers are filtered by object properties.

4) Spring security implemented. You can use basic security credentials to access endpoints.
